...
module('@emberx/string | decamelize', () => {
  test('decamelizes strings', async (assert) => {
    assert.equal(decamelize('my favorite items'), 'my favorite items', 'does nothing with normal string');
    assert.equal(decamelize('css-class-name'), 'css-class-name', 'does nothing with dasherized string');
